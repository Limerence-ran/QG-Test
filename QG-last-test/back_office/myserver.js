...
function handleRequest(request, response) {
    ...
    request.on('end', () => {
        ...
        pool.query(`INSERT INTO books(title, author, description)
                    VALUES('${newBook.title}', '${newBook.author}', '${newBook.description}')`, (error, result) => {
        if (error) throw error;
    ...
    request.on('end', () => {
        ...
        pool.query(`UPDATE books SET title='${updatedBook.title}', author='${updatedBook.author}', description='${updatedBook.description}'
                    WHERE id=${id}`, error => {
        if (error) throw error;
        updatedBook.id = id;
        response.setHeader('Content-Type', 'application/json');
    ...
    } else if (request.method === 'DELETE' && /^\/books\/\d+$/.test(request.url)) {
    // 删除书籍
    let id = request.url.split('/').pop();
...
//已经审核过的书籍
const books = [
    { id: 1, title: 'JavaScript高级程序设计', author: 'Nicholas C. Zakas', publish_time: '2012-08-01' },
    { id: 2, title: 'CSS揭秘', author: 'Lea Verou', publish_time: '2015-06-15' },
    { id: 3, title: 'Node.js实战', author: 'Srephen Duguid & Jonathan Stillman', publish_time: '2014-12-01' }
];
...
const user_information = [
    ...
    //普通用户权限
    {id: 2, user_name:Jiangzr, password: 040707, flag: 0} 
]
...
function handleBooksRequest(req, res) {
    ...
    } else if (requestUrl.pathname.startsWith('/book/') && req.method === 'PATCH') {
    // 修改指定书籍状态
